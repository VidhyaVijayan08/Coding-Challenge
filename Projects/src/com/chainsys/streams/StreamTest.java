...
public class StreamTest {
	public static void main(String[] args) {
		...
		/*****************************************************
		 * 1. Get None student having mobile number 1233 and 1234
		 *****************************************************/
...
//        noneMatch() checks if there are no elements matching the predicate. Here, it simply returns false as soon as it encounters 6, which is divisible by 3.
		...
		List<Student> studs = students.stream().filter(student111 -> student111.getMobileNumbers().stream()
				.noneMatch(x -> Objects.equals(x.getNumber(), "3333"))).collect(Collectors.toList());

		String results = stud2.stream().map(std -> std.getName()).collect(Collectors.joining(",", "[", "]"));
		System.out.println(results);
		System.out.println("--------------------");
		
		
		/*****************************************************
		 * 2. Get Any student having mobile number 1233 and 1234
		 *****************************************************/

//        anyMatch() checks if there are no elements matching the predicate. Here, it simply returns false as soon as it encounters 6, which is divisible by 3.

		List<Student> studens = students.stream().filter(student111 -> student111.getMobileNumbers().stream()
				.anyMatch(x -> Objects.equals(x.getNumber(), "3333"))).collect(Collectors.toList());

		String resultss = stud2.stream().map(std -> std.getName()).collect(Collectors.joining(",", "[", "]"));
		System.out.println(resultss);
		System.out.println("--------------------");
		
		
		/*****************************************************
		 * 3. Get All student having mobile number 1233 and 1234
		 *****************************************************/

//        allMatch() checks if there are no elements matching the predicate. Here, it simply returns false as soon as it encounters 6, which is divisible by 3.

		List<Student> studentss = students.stream().filter(student111 -> student111.getMobileNumbers().stream()
				.allMatch(x -> Objects.equals(x.getNumber(), "3333"))).collect(Collectors.toList());

		String resulttss = stud2.stream().map(std -> std.getName()).collect(Collectors.joining(",", "[", "]"));
		System.out.println(resulttss);
		System.out.println("--------------------");
		
		/*****************************************************
		 * 4. Get student with exact match name "Kayal"
		 *****************************************************/
		Optional<Student> studss = students.stream().filter(student -> student.getName().equals("Kayal")).findFirst();
		System.out.println(studss.isPresent() ? studss.get().getName() : "No student found");
		System.out.println("--------------------");
		
		
		/*****************************************************
		 * 5. Get any student with exact match name "jayesh"
		 *****************************************************/
		Optional<Student> s = students.stream().filter(student -> student.getName().equals("Jayesh")).findAny();
		System.out.println(s.isPresent() ? s.get().getName() : "No student found");
		System.out.println("--------------------");
		

		/*****************************************************
		 * 6. Get all student having mobile numbers 3333.
		 *****************************************************/
		List<Student> stu = students.stream().filter(student111 -> student111.getMobileNumbers().stream()
				.anyMatch(x -> Objects.equals(x.getNumber(), "3333"))).collect(Collectors.toList());

		String res = stu.stream().map(std -> std.getName()).collect(Collectors.joining(",", "[", "]"));
		 ...
		 *****************************************************/
		List<String> nameLists = Arrays.asList("jayesh", "Dany", "Khyati", "Hello", "Mango");

		nameLists.stream().map(String::toUpperCase).forEach(System.out::println);
		System.out.println("--------------------");
		
		/*****************************************************
		 * 8. Conditionally apply Filter condition, say if flag is enabled then
		 *****************************************************/
		boolean sortConditionsFlag = true;

		Stream<Student> conditionalFilterResults = students.stream().filter(std -> std.getName().startsWith("J"));

		if (sortConditionsFlag) {
			conditionalFilterResults = conditionalFilterResults.sorted(Comparator.comparing(Student::getName));
		}

		System.out.println("Before sorting :");
		students.forEach(sa -> System.out.println(sa.getName()));

		List<Student> lists = conditionalFilterResult.collect(Collectors.toList());
		System.out.println("After filter and conditional sorting :");
		list.forEach(sa -> System.out.println(sa.getName()));
